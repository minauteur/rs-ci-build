...
impl Handler for HookH {
    ...
    println!("New payload from: {}", &data);
    match serde_json::from_str(&mut data) {
        Ok(thing) => Ok(thing),
        Err(e) => {
            println!("{:?}", &e);
            Err(PostError::ParseError)
        }
    }
  let keys: &[&str] = &["pull_request", "merged_at"];
  let merged_status = match &mut data.find_path(keys) {
      Some(val) => val,
      None => Err(PostError::ParseError)
    };
    if (merged_status.is_null()) {
      println!("PR hasn't been merged yet!");
      return Err(IronError::new(self,
                               (status::BadRequest, "PR hasn't been merged yet!")))
    }
    else if (!merged_status.is_null()) {
      println!("merged_at: {}", merged_status);
      return Ok(Response::with(status::Ok, &merged_status))
    }
  }
}
// in the middle of 
