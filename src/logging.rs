...
pub trait HasLogger {
    ...
    where
        T: 'static + Send + Sync + RefUnwindSafe + KV;
...
impl<'a, 'b> HasLogger for Request<'a, 'b> {
    fn get_logger(&mut self) -> Rc<Logger> {
        let req_logger = self.extensions.entry::<RequestLogger>().or_insert_with(|| {
    ...
    fn try_get_logger(&self) -> Option<Rc<Logger>> {
        self.extensions().get::<RequestLogger>().map(|logger| {
            match *logger {
                RequestLogger(ref inner) => inner.borrow().clone(),
            }
        })
        ...
        T: 'static + Send + Sync + RefUnwindSafe + KV,
    {
        ...
        self.extensions.insert::<RequestLogger>(RequestLogger(
            RefCell::new(Rc::new(new_logger)),
        ));
    }
